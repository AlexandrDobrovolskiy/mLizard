...
namespace mLizard.Tests
{
    public abstract class mLizardTestBase : AbpIntegratedTestBase<mLizardTestModule>
    {
        ...
        protected void UsingDbContext(Action<mLizardDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<mLizardDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<mLizardDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<mLizardDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<mLizardDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<mLizardDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<mLizardDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<mLizardDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<mLizardDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<mLizardDbContext, Task<T>> func)
        {
            T result;
